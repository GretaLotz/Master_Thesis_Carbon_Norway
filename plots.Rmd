...
### gap plot
Andersson p.14
y axis different scale , +/-0.5 instead of +/-0.2
no arrows for the carbon tax label throughout
```{r wv}
...
#  ylim(-0.8, 1) +
  ...
  #annotate("segment", x = 1987, xend = 1989, y = -0.35, yend = -0.35, colour = "black", arrow = arrow(length = unit(0.1, "inches")))
...
gap_plot_initial <- ggplot(gap_data, aes(x = Year, y = Gap)) +
  geom_line(color = "blue") +  # Line for the Gap
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # Zero line
  labs(title = "Gap in Metric Tons Per Capita (CO2 from Transport)",
       x = "Year",
  ...
  ylim(-0.8, 1) +
  geom_vline(xintercept = 1991, linetype = "dotted", color = "black") +
  annotate("text", x = 1983, y = -0.5, label = "Carbon tax", hjust = 1) +
  annotate("segment", x = 1987, xend = 1989, y = -0.5, yend = -0.5, colour = "black", arrow = arrow(length = unit(0.1, "inches")))

print(gap_plot_initial)
...
### fuel prices
Make this look more like the others, Andersson p.7
Run fuel_prices.Rmd
Andersson hat Steuern bis zu ihrer Einführung auf "Null", der Norwegen-Datensatz hat sie mutmasslich auf NA
Rename full_price to Retail Price
...
plot_disaggregation_wv_1 <- ggplot(data = fuel_prices_diesel, aes(x = year)) +
  ...
  facet_wrap(~"Panel A.\nDiesel", ncol = 1) +
#  facet_grid(. ~ fuel_type, axes = "all", axis.labels = "all_y") +
  labs(title = NULL,
...
# ggsave("plots/fuel_prices_plot.jpg", plot, width = 12, height = 6, dpi = 300)

```

Andersson p.7, text not vertical 
...
### placebo tests
p. 15 
alternative aufmachung von facet graphen
```{r wv placebo tests}
library(cowplot)
...
# Export the plot as a PNG file with a white background
# ggsave("plots/combined_placebo_plots.jpg", plot = combined_placebo_plots, width = 10, height = 6, units = "in", bg = "white")
ggsave("plots/combined_placebo_plots.jpg", plot = combined_placebo_plots, width = 12, height = 6, units = "in", bg = "white")
# ggsave("plots/combined_placebo_plots_wide_2.jpg", plot = combined_placebo_plots, width = 14, height = 6, units = "in", bg = "white")
```
...
gdp_path_plot <- ggplot(gdp_path_data, aes(x = Year)) +
  ...
  labs(title = NULL,
       ...
       linetype = "Legend") +  # Label for linetype legend
  theme_classic() +
  theme(legend.position = c(0.15, 0.13),  # Position the legend at the bottom left
        legend.title = element_blank(),  # Remove the legend title for simplicity
        legend.text = element_text(size = 8),  # Set text size for legend
        legend.background = element_rect(fill = "white", colour = "black"),
        legend.key = element_blank()) +  # No boxes around the legend symbols
...
```{r initial gdp and co2 deviation from synthetic}

# Calculate the range for each variable to set the secondary axis appropriately
gdp_range <- range(combined_data$GDP_Deviation, na.rm = TRUE)
co2_range <- range(combined_data$CO2_Deviation, na.rm = TRUE)
# The ratio will help to align the secondary axis
ratio <- gdp_range[2] / co2_range[2]

# Create the combined plot
combined_plot <- ggplot(combined_data, aes(x = Year)) +
  geom_line(aes(y = GDP_Deviation, color = "GDP Deviation")) +
  geom_line(aes(y = CO2_Deviation * ratio, color = "CO2 Deviation")) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black", size = 0.5) +  # Corrected to horizontal line at y = 0
  scale_y_continuous(
    name = "GDP Deviation",
    limits = range(c(gdp_range, co2_range * ratio)),  # Set limits to accommodate both variables
    sec.axis = sec_axis(~./ratio, name = "CO2 Deviation", breaks = seq(floor(co2_range[1]), ceiling(co2_range[2]), length.out = 5))
  ) +
  labs(title = "Comparison of GDP and CO2 Emissions Deviations from Synthetic Control",
       x = "Year",
       color = "Variable") +
  theme_minimal() +
  scale_color_manual(values = c("GDP Deviation" = "grey", "CO2 Deviation" = "black")) +
  theme(
    legend.title = element_blank(),
    axis.text.x = element_text(angle = 45, hjust = 1),  # To prevent overlapping x-axis labels
...
p.16
Polen oder USA machen hier diesen riesigen Bogen - vielleicht hervorheben? Die werden im nächsten Schritt rausgekickt
Beide plots so wie auch die anderen gap plots
...
Der hier ist ohne Polen und USA
...
# Save the plot with specific dimensions
# ggsave("plots/placebo_in_space_test_gap_plot.jpg", plot = placebo_in_space_test_gap_plot_combined, width = 10, height = 6, units = "in", dpi = 500)
...
```{r initial placebo in-space test, excluding MSPE> 20*Norway, gap plot}

ggplot(data = in_space_gap_data_mspe_20_long, aes(x = Year, y = Gap, group = Country, color = Country)) +
  geom_line() +  # Use line geometry to connect data points over years
  scale_color_manual(values = c("Norway" = "black", "Other" = rep("grey", length(unique(in_space_gap_data_mspe_20_long$Country)) - 1))) +
  labs(title = "Gap in CO2 Emissions Over Years by Country",
       x = "Year",
       ...
       color = "Country") +
  theme_minimal() +  # Use a minimal theme for a clean look
  theme(legend.position = "bottomleft")  # Optionally hide the legend if not needed
```



Horizontaler bar plot p.17
```{r wv}
...
Panel! Panel B like p. 17 leave one out 

leave one out with full sample. highlight excl_luxembourg
Bitte Achsenbeschriftung etc. Kontrollieren, so wie es soll?
...
```
```{r initial leave one out with full sample}

# Now plot using ggplot2
all_leave_one_out_plot_initial <- ggplot(all_significant_leave_one_out_long, aes(x = Year, y = CO2_per_capita, color = Condition)) +
  geom_line() +
  labs(title = "Leave-One-Out Analysis for Synthetic Norway with Significant Weights",
       x = "Year",
       y = "CO2 Emissions per Capita (metric tons)",
...
# Print the plot
print(all_leave_one_out_plot_initial)
...
# Panel A: 3 Linien
# 1.: Norway Black, solid
# 2.: "Synthetic Norway" (aktuelles "Synthetic Norway" aus Panel B (df: path data), black, dashed)
# 3.: "Synthetic Norway (full sample)" (aktuelles Synthetic Norway Panel A, Hell-Grau solid)
...
```{r initial full sample path.plot}

path_plot <- ggplot(full_path_data, aes(x = Year)) +
  geom_line(aes(y = X17, color = "Norway", linetype = "Norway"), size = 0.5) +  # Solid line for Norway (Actual data)
  geom_line(aes(y = Synthetic, color = "Synthetic Norway", linetype = "Synthetic Norway"), size = 0.5) +  # Dashed line for Synthetic control
  geom_vline(xintercept = 1991, linetype = "dotted", color = "black", size = 0.5) +  # Dotted line at the year 1990
  labs(title = NULL,
       x = NULL,
       y = bquote("Metric tons per capita (CO"[2]*" from transport)"),
       color = "Legend",
       linetype = "Legend") +  # Label for linetype legend
  theme_classic() +
  theme(legend.position = c(.15, .13),
        legend.title = element_blank(),  # Remove the legend title for simplicity
        legend.text = element_text(size = 8),  # Set text size for legend
        legend.background = element_rect(fill = "white", colour = "black"), 
        #panel.grid.major = element_blank(),  # Remove major grid lines
...
# Save the plot with specific dimensions
# ggsave("plots/luxembourg_comparison_plot_combined_wide_1.jpg", plot = luxembourg_comparison_plot_combined, width = 10, height = 6, units = "in", dpi = 300)
...
# drei Linien in Panel A: die beiden aktuellen plus die 
# aktuelles Synthetic Norway Panel A: Hell-Grau solid
#



# zwei Legenden, je eins pro panel
# Panel A: 3 Linien
# 1.: Norway Black, solid
# 2.: "Synthetic Norway" (aktuelles "Synthetic Norway" aus Panel B (df: path data), black, dashed)
# 3.: "Synthetic Norway (full sample)" (aktuelles Synthetic Norway Panel A, Hell-Grau solid)
