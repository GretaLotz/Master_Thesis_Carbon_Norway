...
# Aggregate data by country and indicator, summing across years
aggregated_data <- filtered_data %>%
  group_by(country, country_code, indicator, indicator_code) %>%
  summarize(across(where(is.numeric), sum, na.rm = TRUE), .groups = "drop")

# Calculate sum across year columns for the total fuel combustion row
...
# Combine aggregated data with new total combustion row
...
```{r}
# Filter data for transport emissions (% of total fuel combustion) indicator
...
# calculate absolute of kt CO2 from percentage and total kt of fuel combustion
norway_long <- norway_long %>%
...
# Filter data for Norway 1960-2005
...
# Filter data for indicator code EN.POP.DNST for Norway
